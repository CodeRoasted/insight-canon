...
// build-only local in canon's tokenizer/parser impl units — never the api surface, so no level macro
// leaks through the module.) This header stays PUBLIC/installed because downstream packages use the
// INSIGHT_LOG_* macros.
#pragma once

// SPDLOG_ACTIVE_LEVEL is the compile-time elision threshold. A TU that wants a specific level sets it
// as its OWN PRIVATE -D (canon does, per build type); canon does NOT export its level PUBLIC — a
