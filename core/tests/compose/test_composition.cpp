...
// test_composition.cpp — the composition CONTRACT (ADR 0024 §3/§4), canon's semantic-unaware machinery,
// over SYNTHETIC manifests. Three permanent properties the §10 gates name:
//   • G-SP-5 fail-closed — an exact-duplicate key across packages is a BUILD error (constexpr
//     find_conflict, static_assert'd here) AND a startup FATAL (the runtime compose, EXPECT_DEATH'd).
//   • The degenerate core-only composition (compose({})) is a defined, runnable state: universal formats
//     tokenize, no dialect row fires, the identity is the stable hash of just the version components.
//   • semantic_identity is a reproducible, order-independent, CONTENT hash — the II-7 comparability key
//     that REPLACES the retired kIntentRegistryVersion literal (ADR 0024 §4.1). This is the unit-level
//     G-SP-4 guard; the cross-build / cross-OS leg is Argos's CI (det_public_proof).
// Determinism: byte-only; the hash is truncated-SHA-256 over a fixed-endian canonical serialization,
// order-independent by construction (compose sorts packages by name). No RNG/clock/float.
...
constexpr std::array<StructuralRoleRow, 1> kRolesAMut{{
    {.prefix = "<AAB>", .role = StructuralRole::GroupBegin, .format_gate = kAnyFormat}}}; // one byte changed
constexpr std::array<StructuralRoleRow, 1> kRolesB{{
    {.prefix = "<BBB>", .role = StructuralRole::GroupEnd, .format_gate = kAnyFormat}}};

constexpr SemanticPackageManifest kPkgA{
    .name = "alpha", .version = "1.0.0", .roles = kRolesA, .markers = {}, .level_lifts = {},
    .locations = {}, .value_classes = {}, .strategy = nullptr, .echoed_source = nullptr};
constexpr SemanticPackageManifest kPkgAMut{
    .name = "alpha", .version = "1.0.0", .roles = kRolesAMut, .markers = {}, .level_lifts = {},
    .locations = {}, .value_classes = {}, .strategy = nullptr, .echoed_source = nullptr};
constexpr SemanticPackageManifest kPkgB{
    .name = "beta", .version = "1.0.0", .roles = kRolesB, .markers = {}, .level_lifts = {},
    .locations = {}, .value_classes = {}, .strategy = nullptr, .echoed_source = nullptr};

// ── A deliberate cross-package DUPLICATE: two packages sharing an exact role key with intersecting gates
// (kAnyFormat intersects everything). The G-SP-5 fixture. ──
...
constexpr SemanticPackageManifest kDupA{
    .name = "dup_a", .version = "1.0.0", .roles = kDupRoleX, .markers = {}, .level_lifts = {},
    .locations = {}, .value_classes = {}, .strategy = nullptr, .echoed_source = nullptr};
constexpr SemanticPackageManifest kDupB{
    .name = "dup_b", .version = "1.0.0", .roles = kDupRoleY, .markers = {}, .level_lifts = {},
    .locations = {}, .value_classes = {}, .strategy = nullptr, .echoed_source = nullptr};
constexpr std::array<SemanticPackageManifest, 2> kDupSet{kDupA, kDupB};
} // namespace

// ── G-SP-5, build-time half: find_conflict is constexpr, so a duplicate is caught in a static_assert ──
static_assert(find_conflict(kDupSet).has_conflict,
              "G-SP-5: a cross-package exact-duplicate role key must be detectable at compile time");
static_assert(find_conflict(kDupSet).kind == "role", "the conflict must be reported as a role duplicate");
...
// ── G-SP-5, runtime half: compose() on the duplicate set FAILS CLOSED (fatal) — never a silent merge ──
...
// ── The degenerate core-only composition is a defined, RUNNABLE state (II-4 at the composition layer) ──
TEST(Composition, DegenerateCoreOnlyRuns)
{
    ...
    // No dialect row fires — a would-be GHA marker classifies to None, no marker, no test-file WHERE.
    EXPECT_EQ(classify("##[error]boom", LogFormat::GitHubActions, core), StructuralRole::None);
    EXPECT_EQ(recognize("Run something", LogFormat::GitHubActions, core).kind, IntentMarkerKind::None);
    ...
    EXPECT_EQ(event->format, LogFormat::JSON) << "a JSON line must still tokenize under a zero-package composition";
}
...
TEST(Composition, SemanticIdentityIsReproducibleOrderIndependentAndContentSensitive)
{
    ...
    EXPECT_EQ(compose(oneA).identity(), compose(oneA).identity())
        << "identity must be a pure function of the package set (got " << compose(oneA).identity_hex()
        << " then a different hash)";

    // Order-independence: compose({A,B}) == compose({B,A}) — canonical order is by name, not argument order.
    ...
    // Content-sensitivity: mutating ONE row byte changes the identity (content hash, not a version label).
    ...
    // The degenerate composition has a stable, non-trivial identity (hash of the version components).
