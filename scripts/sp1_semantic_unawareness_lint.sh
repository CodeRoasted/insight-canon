#!/usr/bin/env bash
###############################################################################
# The semantic-unawareness lint (ADR-17.D1; insight_canon_semantic_packages.md §9)
#
...
# an ADR-17.D1 regression = FAIL. Verbose on failure (file:line of every hit).
...
echo "ADR-17.D1 semantic-unawareness lint — scanned: ${scanned} files under ${SCAN_ROOTS[*]} (core mechanism + contract surface)"
echo "deny-list (behavioral ecosystem literals; LogFormat enum names deliberately excluded):"
printf '    %s\n' "${DENY[@]}"
echo

if [ "$scanned" -eq 0 ]; then
  echo "::error::semantic-unawareness lint scanned ZERO files — the scan roots (${SCAN_ROOTS[*]}) are missing or empty."
