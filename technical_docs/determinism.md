...
`canonicalization_version` (owned by `kCanonicalizationVersion` in `core/api/canon.api.cppm` — the
current value and the generation ledger live there, never mirrored here) is the contract that makes templates
