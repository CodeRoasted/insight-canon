...
2. **The measurement that justified splitting the two counters has no home — Eqya, to route.**
   ...
   DISCHARGED 2026-09-12 `insight-canon 0ea8c1c` — re-run, and the numbers now live in an arm that re-derives them on every corpus-gate run: `TransportPeelEquivalenceGate.NoEventLinesLandOnTheSkipCounterAndNeverOnTheFailureCounter` (`core/tests/transport/test_transport_peel_equivalence_gate.cpp`), the file the `skipped_count_` invariant's `refs:` already names. Coordinate: the population that file reads from the manifest (sorted, uncapped), the shipped one-row `api-rfc3339-line-prefix` declaration, each peeled line through `LogParser::parse_line` (the parse under the facade's `process_line` and so under `InsightPipeline::ingest_peeled_line`), one parser per log, the zero-package composition (a composed dialect may route a line elsewhere); a refusal whose reason starts `LogParser: ` is a no-event line, any other a strategy failure keyed by its `<Name>Strategy` prefix. **`data/v1/full`, 4 082 logs, 22 490 937 lines: 21 841 845 parsed, 561 809 no-event lines, 87 283 strategy failures** (86 226 `JsonStrategy`, 602 `KVStrategy`, 455 `IISW3CStrategy`), so an unsplit counter would be 86.6 % no-event input. `data/v1/sample`, 60 logs: 293 955 / 11 737 / 160. The effect of the split, replayed at the WARN's cadence (the first failure and every 100th, per parser) and LOGGED rather than pinned because the cadence is private to `log_parser.cpp`: **1 558 bounded failure reports under the split counter against 825 under an unsplit one** — `IISW3CStrategy` 106 against 2, `KVStrategy` 147 against 2, `JsonStrategy` 1 305 against 821. The deleted figures (87 643 failures, 523 126 empty lines, 837 → 1 439 reports, IIS 0 → 47, key=value 0 → 1) are NOT reconciled: they were taken on older code, before whitespace-only lines left the failure counter (`insight-canon dfe4f4e`), under an unrecorded composition. The arm asserts the partition closes, that each log's `lines_failed()` equals its strategy refusals, and the three counts per slice. Red with the empty-line seat of `parse_line` moved onto `failed_count_`: 49 of 60 sample logs and 3 640 of 4 082 full logs disagree, `lines_failed()` 610 409 against 87 283 refusals — a gap of exactly the 523 126 blank-and-empty lines the peel gate pins; restored by byte copy, `sha256sum -c` OK. Both slices green on clang-21 and gcc-16.2 (full slice 26–32 s clang, 20 s gcc); `malf test insight-canon` core 755/755, semantic 33 + 25 + 15 + 6 on both toolchains; `malf format --check insight-canon` 128 files, 0 misformatted, 0 CCC violations.
3. **A test caption states a rule the code does not implement —
   ...
   DISCHARGED 2026-09-13 `insight-canon 8c33570` for the residual half (Kleio): `FormatDetectorTest.TheBatchWinnerIsTheConfidenceSumNeitherTheCountNorTheStrongestLine` (`core/tests/parse/test_format_detector.cpp`), homed as a unit arm because the rule is one component's function of its sample. It first asserts that no built-in strategy scores the fixture lines, then registers two fixed-confidence strategies on the zero-package detector. Two lines at 0.25 against one at 0.625 must name the strong line's format (sum 0.625 > 0.5, count 1 < 2); three lines at 0.25 against one at 0.625 must name the weak lines' format (sum 0.75 > 0.625, while the strongest single line is still 0.625). Red on clang-21 under each rival rule, restored by byte copy with `sha256sum -c` OK: a count of claimed lines reds the first sample (winner HPC), and keeping each format's strongest line reds the second (winner Proxifier). `BatchHandlesMixedFormats` stays green under both, which is why it never separated the rules; `BatchDetectsJSON`'s comment calling the rule a majority is corrected in the same commit. Counts: core 765/765, github 33/33, gitlab 25/25, jenkins 15/15, test_frameworks 6/6 on clang-21 and on gcc-16.2; `malf format --check insight-canon` 128 files, 0 misformatted, 0 CCC violations.
...
1. **`DN-46.D1`'s table row and its action list assert a source comment this unit deletes —
   ...
   *"DONE, verified at source"* on the strength of it. CCC deletes the sentence: it is a claim about
   ...
   REFUSED 2026-09-12 moot, re-derived by Daidalos (W20) — `DN-46` left the design-note shelf on 2026-09-10 in `coderoast 2a3ef665`, deleted after draining into `ADR-23`, and that commit's map files `DN-46.D1` as a record rather than a graduated slot, so no live document carries the stale row or its action list. The claim the row recorded — the transforms a stream declared ride the MetaLog document's `extensions` container as `fr.coderoast.transport` — is stated at `ADR-23.D4` (`adr/023-insight-transport-envelope.md`, line 166 at HEAD), and `core/src/compose/compose.cpp` cites `ADR-23.D4` on three `refs:` lines (229, 251, 369). A workspace sweep for `DN-46`/`DN-046` by the root `CLAUDE.md` recipe, attic excluded, finds only this ledger, `DONE.md`, one dated conformance report on the frozen audit shelf and the ungated `.claude/shelf_write_authorized`; none is a design-tier surface.
...
2. **The `export import insight.canon.spi` in that file has no in-repo witness.** Its stated reason
   ...
   WITNESSED 2026-09-13 (Kleio) for the github, gitlab and jenkins packages the line above reported unwitnessed, so no arm is owed. With `export import insight.canon.spi` made a plain import in all three (clang-21, each package built alone, restored by byte copy, `sha256sum -c` OK), existing test units already fail to compile because they name canon's types through the package module alone (`declaration of 'kAnyDialect' must be imported from module 'insight.canon.spi'`, `no member named 'SemanticPackageManifest'`): github `test_github_declared_ingest.cpp`, `test_github_markers.cpp`, `test_github_outcome.cpp`, `test_github_roles.cpp`, `test_github_round_trip.cpp`; gitlab `test_gitlab_markers.cpp`, `test_gitlab_package_corpus_proof_gate.cpp`, `test_gitlab_round_trip.cpp`; jenkins `payload_stamp_template_count_measurement_test.cpp`, `test_jenkins_markers.cpp`, `test_jenkins_outcome.cpp`, `test_jenkins_round_trip.cpp`. Each list is what the stopped build logged, so it is a lower bound. A dedicated surface arm per package was written, measured redundant, and deleted uncommitted.
...
1. **The corpus oracle's six unread columns name an obligation nothing in the tree discharges.**
   ...
   DISCHARGED 2026-09-13 `insight-canon 855433b` + `coderoast-corpora effbc4e` — the six columns are DROPPED, as Eqya ruled 2026-09-12: an unread output is dormant plumbing, and a gate built only to justify it would be premature specialization. `emit_recognition_delta.py` writes canon's five columns to the file and keeps the eidos-side cells in its stdout summary, which still reproduces all nine recorded figures; the committed TSV was regenerated from `data/v1` (627 trace rows) and is byte-identical to `cut -f1-5` of the previous file, and this gate's frozen header and field count follow. `GitLabPackageCorpusProofGate` 3/3 against the mounted corpus; `malf test insight-canon` core 764/764, github 33/33, gitlab 25/25, jenkins 15/15, test_frameworks 6/6 on clang-21 and gcc-16.2.
   PARTIAL. DISCHARGED 2026-09-13 for Eqya's half, by ruling, on the residual question left NOT decided above. Measured before ruling: the harness's post-seam path (canon's ANSI strip, then eidos's `strip_leading_timestamp`) recognizes 3 487 markers against canon's 3 487 over all 627 trace rows, 0 lost and 0 gained, at `coderoast-corpora effbc4e` on the mounted `data/v1`. It is a Python model, so it corroborates and gates nothing. Ruled: (a) the residual stays and becomes a CHECKED reproduction figure (0 lost, 0 gained), and the script exits non-zero on any MISMATCH, so the projection-freshness step that already runs it reds on a drift — an unread stdout line checks nothing; (b) the pre-seam path, which models code deleted at `insight-eidos d4986bf`, is ripped with its five pre-seam figures, which reproduce a historical defect no live surface cites; (c) the script, its test and the committed file are renamed from `recognition-delta`, which the file no longer is, to `canon-recognition` (`emit_canon_recognition.py`, `PROBE-v1.canon-recognition.tsv`), cascading in one pass to this gate's `kOracleFile`, `scripts/run_corpus_gates.sh`, `.github/workflows/corpus-gates.yml`, the marker probe README, `emit_outcome_census.py`'s import and the GHA harness's docstring. Owed to Hephaïstos, with the build slot, this gate green on clang-21 and gcc-16.2 against the mounted corpus.
   DISCHARGED 2026-09-14 `coderoast-corpora 8198db2` + `insight-canon 4badcb1` + superproject `e54ed89f` — Hephaïstos's half, as ruled. (a) The post-seam residual is a checked figure and `emit_canon_recognition.py` exits 1 on any mismatch among 9 checked figures: 482 stamped traces, canon 3 193 stamped, study denominators 3 231 and 1 054, canon 3 487 and eidos post-seam 3 487 over all 627 trace rows, 0 lost, 0 gained, 0 paren payloads; measured on `data/v1`, exit 0 with all 9 matching, and exit 1 naming the figure when one recorded value was mutated. (b) The pre-seam path and its five figures are ripped. (c) The script, its test and the committed file are `emit_canon_recognition.py`, `test_canon_recognition.py` and `PROBE-v1.canon-recognition.tsv`, the file byte-identical; this gate's `kOracleFile`, `scripts/run_corpus_gates.sh`, `.github/workflows/corpus-gates.yml`, the probe README, `emit_outcome_census.py`'s and the GHA harness's imports and `scripts/registry_grammar_lint.py`'s note follow. `GitLabPackageCorpusProofGate` 3/3 against the mounted corpus on clang-21 and gcc-16.2; shape tests 34/34.
...
* **The determinism waiver token** that the `wallclock` and `random_determinism` check modules read —
...
* **`parse_template_id` is silently tolerant of malformed input** (reader A, Q5). It accepts a
  ...
  DISCHARGED 2026-09-13 `insight-canon 0b4b6f3` + `insight-metalog f363d68` + `insight-eidos 23ce5351` — ruled a PARSER that fails closed (`ADR-16.D3`). `parse_template_id` accepts exactly `h:` plus 32 lowercase hex digits and returns `std::expected<TemplateId, TemplateIdParseError>` (`MissingPrefix`, `WrongLength`, `NotLowercaseHex`); the label tolerance moved to the declared fixture constructor `template_id_of_label`, which derives the id from the label's bytes under `template_id_of`'s hash. The three callers: the metalog compose fixture takes the label constructor, the eidos alert-fold gate fails its arm on a refused id, and canon's round-trip test asserts the value. Arms `TemplateIdInvariants.ParseRefusesEveryMalformedForm` (11 inputs, each with its reason) and `.LabelConstructorSeparatesShortLabels`, red under the parent's tolerant body: `""` parsed as `h:0000…`, and `h:abc`, `h:abd`, `h:ab` and `abc` collided on one id. Counts on clang-21 and gcc-16.2: canon core 764/764 (semantic 33, 25, 15, 6); metalog 308/313, the 5 being the by-design `ComposeShardFold` reds; eidos root 356/356.
...
change with a corpus cost. **Addressee: the `insight-canon` utils lane, and Eqya for the disposition.**
  ...
  PARTIAL. DISCHARGED 2026-09-12 for the corpus count, by measurement (Kleio) — ZERO lines on both questions. No corpus line carries a date field whose day exceeds its month's length for its year (Feb 29 in a non-leap year, Feb 30/31, Apr/Jun/Sep/Nov 31): not in the shape `parse_iso8601` reads, and not in the field shape of any of the other eight `utc_mktime` callers (`parse_log4j_timestamp`, `parse_nginx_error_ts`, `parse_short_year_slash`, `parse_health_app_ts`, `parse_compact_date_time`, `parse_clf_timestamp`, `parse_apache_error_ts`, `parse_bsd_syslog_ts`; a BSD `Feb 29` occurs nowhere, so that count holds for any reference year). Denominators, as lines carrying at least one field of the shape, on disk 2026-09-12: the `scripts/run_corpus_gates.sh` corpora — ci-revert `v1/full` 44 586 257 lines (44 379 247 ISO-shaped, 208 778 slash, 59 570 month-name, 2 732 compact) and `v1/sample` 602 851 (598 822 ISO); gitlab-markers `v1` 6 939 906 (3 535 698 ISO); jenkins-markers `v1`+`v2` 824 056 (171 760 ISO, 180 month-name); loghub-full `BGL.log`+`Thunderbird_5M.log` 9 747 972 (4 899 435 month-name, 8 722 compact, 1 283 slash) — plus every other byte root of `coderoast-corpora`, the longitudinal bank included (114 303 files, 501 680 862 lines, 496 908 653 ISO). Method: a byte-level text scan, not an instrumented run — a prefilter wider than each parser's accepted field language at ANY offset, then the exact per-parser shape with the calendar check; rescanned after canon's ANSI strip on the 86 917 ESC-bearing files, and for the JSON-escaped, UTF-16, sign-prefixed short-year and frozen `.tar.zst` forms, all zero. It is an UPPER bound (it counts dates no parser reaches), so its zero is exact; it cannot see a corpus outside those roots. Controls: over the tracked tree the same classifier finds exactly the three arms that pin today's normalisation — `ParseISO8601.LeapDayFeb29NonLeapYearNormalises`, `ParseBSDSyslog.OutOfRangeDayNormalises`, `SyslogStrategyTest.InvalidDayLineDoesNotCrash` — and rejects the valid 2024, 1984 and 1972 leap days the bank carries. So the refusal moves zero corpus lines and no corpus gate or golden; those three arms flip with the code. For the fix, read at source and not run: `utc_mktime`'s existing out-of-range return is the epoch value 0, which all nine callers wrap as a PRESENT timestamp — both BSD arms assert only `has_value()` and would stay green on it, and `parse_iso8601` and `parse_clf_timestamp` add the zone offset to it, publishing an instant near 1970 — so a days-in-month guard that reuses that return is not a refusal. Hephaïstos owes the refusal.
  DISCHARGED 2026-09-12 `insight-canon 0f8e3cf` — the refusal, after Kleio's count (`insight-canon 4f16034`, zero corpus lines). `utc_mktime` now returns `std::optional<std::time_t>` and refuses a day past its month's length (Gregorian leap rule) together with the existing month and year guards, so the refusal is an ABSENCE every caller answers as `nullopt`, never the epoch value 0 — re-derived at source before the change: the old sentinel reached all nine callers as a present timestamp, and `parse_iso8601`/`parse_clf_timestamp` added the zone offset to it. The `CanonicalEvent` absence sentinel `Timestamp{}` (`tokenizer_engine.cpp`'s `value_or(Timestamp{})`) is unchanged, so downstream absence reads as before. Arms, each RED against the parent `time_utils.cpp` (run 2026-09-12, 0/5 passed) and green after: `ParseISO8601.DayPastItsMonthIsRefused` (parent: `2023-02-29T00:00:00+02:00 is no instant, yet it parsed to epoch second 1677621600`), `ParseISO8601.MonthOutOfRangeIsRefusedWithAnOffset` (parent: `2023-13-01T00:00:00+02:00 ... parsed to epoch second -7200` — the sentinel-plus-offset leak, measured), `ParseCLF.DayPastItsMonthIsRefusedWithAnOffset`, and the flipped `ParseBSDSyslog.OutOfRangeDayIsRefused` (parent: `Feb 30 parsed to epoch second 1709294400`) and `SyslogStrategyTest.InvalidDayLineParsesWithNoTimestamp` (parent: `an impossible date published a timestamp`); `LeapDayFeb29NonLeapYearNormalises` is replaced by the refusal arm. `malf test insight-canon`: core 755/755 (one disabled regenerator) + semantic 33, 25, 15, 6 on clang-21 and on gcc-16.2; `kCanonicalizationVersion` stays `stateless-masks-15` and no golden moved. NOT in this ruling and left as found: the clock fields are not range-guarded (an hour of 25 still normalises into the next day) — its corpus cost is uncounted.
  PARTIAL. DISCHARGED 2026-09-12 for the clock-field corpus count, by measurement (Kleio) — ZERO reachable lines, so a refusal of an out-of-range clock field moves no corpus line, gate or golden; the refusal itself (Eqya's ruling, the dates' precision-first disposition) stays owed to Hephaïstos. Read at source first (`time_utils.cpp` at `insight-canon c5dbc36`): every caller reads hour, minute and second as two digits (one or two in `parse_health_app_ts`) and `utc_mktime` adds `hour·3600 + minute·60 + second` with no range check, so an hour of 25 rolls into the next day and a second of 60 into the next minute. Counted per class, on a line whose date passes today's guards (a line with an impossible date is already refused): an OUT-OF-RANGE field (hour > 23, minute > 59, second > 60), and a second of exactly 60 kept APART as the leap-second question, split into `23:59:60` and any other `:60`; plus, adjacent and reported, a zone offset past 23 hours or 59 minutes in the two parsers that read one (`parse_iso8601`, `parse_clf_timestamp`). Same method and roots as the date count above: a prefilter wider than every caller's clock language at ANY offset (a two-digit field of 24–99 in the hour slot, 60–99 in the minute or second slot, the compact `HHMMSS` form, and the two offset forms), then the exact per-parser shape of all nine `utc_mktime` callers with the date check and the clock classes, over `/home/windows/corpora-data` (64 646 002 lines), the longitudinal bank (114 303 files, 501 680 862 lines) and `coderoast-corpora` (111 224 files, 566 469 258 lines, which mirrors the bank), 2026-09-12; rescanned after canon's ANSI strip on the 86 917 ESC-bearing files, with the NUL bytes stripped from the 26 UTF-16 files (208 515 ISO-shaped lines visible through that decode), in the 8 frozen `.tar.zst` archives, and for a JSON-escaped colon (`\u003a` between digits: 0 lines in all three roots). Result: 0 lines in the leap-second class (no `:60` second in any date shape, so taking 59 or 60 as the largest second costs the corpus nothing either way), 0 zone offsets, 0 out-of-range fields for eight of the nine parsers; the upper bound finds 222 raw lines, all in `parse_compact_date_time`'s `YYMMDD HHMMSS` shape and all false positives — digit columns of a numeric table (a GitLab job's `Total` row, 26 Stirling-PDF logs' shared-object sizes and their store mirror, a GitLab trace-sidecar TSV, one `BGL.log` line) at byte offsets 66 to 206, while that parser's one caller (`spark_hdfs.cpp`) reads the date at offset 0 and the time at offset 7 of the line, so none is reachable. Controls: a synthetic file carrying one invalid clock per parser shape, both `:60` cases, a bad zone offset and an impossible date is classified exactly (13 flagged: 10 out of range, one per `:60` case, one zone offset; 1 refused as a date; 2 valid lines passed), and the ANSI rescan finds only the raw pass's lines again. It is an upper bound that cannot see a corpus outside these roots.
  DISCHARGED 2026-09-13 `insight-canon 65c9fd8` — the refusal (W21), after Kleio's count above (0 reachable lines in every class). `utc_mktime` now refuses an hour outside 0..23 and a minute or second outside 0..59 through the no-value path the impossible dates took in `0f8e3cf`, so all nine callers answer `nullopt`; a signed two-digit field (`-1`, which the standard conversion accepts) is refused by the same bounds. **The largest second is 59, not 60:** `time_t` counts no leap second, so `23:59:60` could only be published as the next day's `00:00:00`, an instant other than the one written, and the count found no `:60` second to lose. The two parsers that read a zone offset (`parse_iso8601`, `parse_clf_timestamp`) applied it unguarded; `utc_correction` refuses an offset hour outside 0..23 or minute outside 0..59, signed fields included. Arms, each RED against the parent `time_utils.cpp` on clang-21 (6 of 6 failed over 44 inputs, each printing the instant it published) and green after: `ParseISO8601.ClockFieldOutOfRangeIsRefused` (parent: `2023-06-15T25:00:00+02:00` parsed to epoch second 1686870000), `ParseISO8601.LeapSecondIsRefused` (`2016-12-31T23:59:60Z` to 1483228800), `ParseISO8601.ZoneOffsetOutOfRangeIsRefused` (`+24:00` to 1686744000), `ParseBSDSyslog.ClockFieldOutOfRangeIsRefused` (`Jun 15 -1:00:00` to 1718406000), `ParseCLF.ClockFieldOrZoneOffsetOutOfRangeIsRefused` (`15/Jan/2024:24:00:00 -0700` to 1705388400) and `DialectTimestampCalendar.AClockFieldOutOfRangeIsRefusedByEveryParser` over the other six callers (nginx hour 24 to 1705365000). `malf test insight-canon`: core 761/761 (one disabled regenerator) + semantic 33, 25, 15, 6 on clang-21 and on gcc-16.2; `kCanonicalizationVersion` stays `stateless-masks-15` and no golden moved. Left as found, uncounted: an offset whose digits do not parse (`+ab:cd`) is still read as 0, because `parse2d`'s and `parse_fixed`'s results are ignored at both offset sites.
