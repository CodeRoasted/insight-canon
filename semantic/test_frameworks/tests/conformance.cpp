...
// manifest. Package-agnostic canon-shipped gate; this is the entire per-package instantiation. A failing
// check prints its own verbose diagnostic. Determinism: seedless, single-threaded, pure over manifest data.
...
{
    const auto report{insight::semantic::conformance::run(insight::semantic::test_frameworks::kManifest)};
