...
// the echoed-source provenance hook + the dialect's private byte primitives
// (is_github_actions_prefix, kGhaPrefixLen, the command-echo SGR grammar). Relocated from canon
// core (src/strategy/github_actions.cpp
...
// (G-SP-1). The universal sub-primitives (ISO date / time digit shapes) are re-homed here as
// private helpers rather than depending on core's sealed scan shard.
...
namespace
{
    ...
    // ── Dialect line-prefix detection (was src/scan/canon.detail.scan.cppm
    // is_github_actions_prefix) ──
    ...
    {
        return match_iso_date_at(str, 0) && str.size() > 10U && str[10] == 'T' &&
               match_time_at(str, 11U);
    }

    // "YYYY-MM-DDTHH:MM:SS.fffffffZ" — GHA / Azure Pipelines line prefix (exactly 7 fractional
    // digits + 'Z'). A strict subset of RFC 3339 so the strategy outranks Syslog only on genuine
    // GHA lines.
    ...
    class GitHubActionsStrategy final : public insight::tokenization::IFormatStrategy
    {
      public:
        ...
        parse(std::string_view line, insight::tokenization::ArenaAllocator& arena) const override
        {
            ...
            // Everything past the fixed-width timestamp; drop the separator space + any GHA
            // indentation.
...
// (`36;1`/`1;36`), a content run, and a closing reset (`0`/empty/`39`) — no un-wrapped visible
// bytes outside the span. Operates on the RAW line (ANSI intact). Byte-exact state machine (F5).
// Ported verbatim from the pre-split canon scan shard; the composed provenance hook LogParser
// consults.
