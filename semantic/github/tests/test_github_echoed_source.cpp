...
// test_github_echoed_source.cpp — the GitHub-Actions echoed-source CODE TIER (ADR 0024 §2.3.2, D-PROV-1).
// Migrated from canon tests/parse/test_echoed_source.cpp. Two altitudes:
//   1. RECOGNITION — github::is_echoed_source(raw): the byte-exact command-echo SGR predicate, relocated
//      into this package (the `\x1b[36;1m … \x1b[0m` grammar is GHA dialect knowledge).
//   2. THE GATE — the composed provenance hook, threaded through the PUBLIC Tokenizer (compose({github})),
//      demotes an echoed failure line's level to Unknown and sets CanonicalEvent.echoed_source. This is a
//      composed integration: canon's parser layer consulting THIS package's hook.
// A red (`31`) coloured REAL error is byte-distinct from the command-echo SGR, so it is never swallowed —
// the precision face. Determinism: byte-exact state machine, no RNG/clock/float.
...
{
    EXPECT_TRUE(is_echoed_source("\x1b[36;1m    echo \"Download failed after 3 attempts\" >&2\x1b[0m"))
        << "the GHA command-echo of an `echo \"… failed …\"` script line";
    EXPECT_TRUE(is_echoed_source("\x1b[36;1mset -e\x1b[0m")) << "a wrapped script line, no timestamp";
    ...
    EXPECT_TRUE(is_echoed_source("\x1b[36;1m\x1b[0m")) << "an empty wrapped span — a blank script line";
...
TEST(GithubEchoedSource, NonCommandEchoLinesAreNotEchoedSource)
{
    ...
    EXPECT_FALSE(is_echoed_source("\x1b[36;1mset -e")) << "no closing reset — not a clean wrapped span";
...
// The github provenance hook, composed into the tokenizer, demotes an echoed `… failed …` script line so
// its failure WORD confers no alerting level.
TEST(GithubEchoedSource, TokenizerDemotesEchoedFailureLevelToUnknown)
{
    ...
    const auto echoed{tokenizer.process_line(gha("\x1b[36;1m    echo \"Download failed after 3 attempts\" >&2\x1b[0m"))};
