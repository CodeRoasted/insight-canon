...
{
    return (chr >= 'a' && chr <= 'z') || (chr >= 'A' && chr <= 'Z') || is_digit(chr) || chr == '_';
}
...
// The timestamper-plugin prefix: `[` + a STRICT RFC3339 timestamp (`YYYY-MM-DDTHH:MM:SS`, optional
// `.fff…` fraction, optional `Z` / `±HH:MM` / `±HHMM` zone) + `]`. Returns the offset ONE PAST the
// closing `]`, or 0 when the line does not carry the prefix. Strictness is the anti-phantom guard:
// a Proxifier `[10.20.30.40]`, an ApacheError `[Mon Oct 03 …]`, or a bare `[12:34:56]` never match.
...
// timestamper strip (a timestamper-prefixed line is claimed REGARDLESS of its content — the strip
// itself is the dialect knowledge, and it is what restores template collapse on ci.jenkins.io
// consoles).
